...
async function syncProjectStatuses() {
  // 1. Sync from local storage immediately
  ['01', '02', '03'].forEach(num => {
    const status = localStorage.getItem(`ag_proj_status_${num}`);
    applyBadgeUI(num, status);
  });

  // 2. Sync live status from server for cross-device updates (Phone <-> Laptop)
  try {
    ...
    if (data.success && data.roadmap) {
      data.roadmap.forEach(item => {
        ...
        if (num && item.status) {
          localStorage.setItem(`ag_proj_status_${num}`, item.status);
          applyBadgeUI(num, item.status);
        }
      });
    }
  } catch (e) {}
